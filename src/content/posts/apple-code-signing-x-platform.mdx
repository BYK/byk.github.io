...
See, all I wanted to do was to [create a terminal application](/posts/fossilize#a-wild-boss-appears-signing-and-notarizing-on-macos) that you can just download and run on Linux, macOS, and Windows.
...
The very first thing Apple wants is ~~your money~~ an Apple Developer account which will [set you back $99](https://developer.apple.com/programs/whats-included/)[^1]. Every year that is.
Oh, and you cannot _just_ create a developer account. You see, you need One Apple Account™.
If this is going to be a personal developer account, you _just_ need your name, email, phone number, and your address[^2]. If you are trying to enroll your organization, god help you: you need your [D-U-N-S number](https://developer.apple.com/help/account/membership/D-U-N-S/).[^3]
...
Okay head over to [identifier creation page](https://developer.apple.com/account/resources/identifiers/bundleId/add/bundle) and get it over with please. I'll `await`.
I _think_ you leave the capabilities empty.

## Switching to the Highway

After this step we need to add a certificate to our account. Now, if you have XCode, there's a built in UI for this. But remember, we don't have access to macOS where XCode can only survive in. Hence we will go
rogue and will create a certificate signing request (CSR) using the command line. We need a "private key" to create a CSR so we'll be creating that via the CLI too.
...
1. `openssl`[^4]
1. [`rcodesign`](https://github.com/indygreg/apple-platform-rs/releases/latest)
...
Okay, we have the signing request in `csr.pem`. Now we head to the page where you can [add a certificate](https://developer.apple.com/account/resources/certificates/add)
...
If you enjoy a good read from Apple go [read their documentation](https://developer.apple.com/documentation/appstoreconnectapi/creating-api-keys-for-app-store-connect-api).
For the twitchy ones, like myself:

1. Head to [API Key Creation](https://appstoreconnect.apple.com/access/integrations/api) page
...
copied](https://github.com/BYK/fossilize/blob/main/entitlements.plist) what Node.js used for itself.

You can use this or create your own by picking and choosing from [the vast array of entitlements](https://developer.apple.com/documentation/bundleresources/entitlements)
that Apple offers. There's also [more excellent prose](https://developer.apple.com/documentation/security/notarizing-macos-software-before-distribution)
...
[properly formatted](https://developer.apple.com/documentation/security/resolving-common-notarization-issues#Ensure-properly-formatted-entitlements)[^5] next to the binary
...
1. I don't think you need to password-protect your p12 file but if you are using a service like GitHub you probably cannot store files as secrets. A quick hack for
   ...
   [`base64` decode these into their respective files](https://github.com/getsentry/spotlight/blob/4f3e34a43e5d1949f664fc8ea88f84b1050274af/.github/workflows/build.yml#L61-L65)
   and continue business as usual.
1. Also, don't forget to [store the _signed_ binary](https://github.com/getsentry/spotlight/blob/4f3e34a43e5d1949f664fc8ea88f84b1050274af/.github/workflows/build.yml#L96-L101) as the artifact of your build.

## Resources

I've used the excellent docs Gregory Szorc created for his amazing [`apple-codesign`](https://gregoryszorc.com/docs/apple-codesign)(`rcodesign`) project.
...
I also found [this amazing gist for creating `pkpass.p12` files](https://gist.github.com/karnauskas/f76ab849224f22fc32961288266094a2) from the GitHub user
[karnauskas](https://gist.github.com/karnauskas) and used parts of it.

Finally, I've used [this little hack from StackOverflow](https://stackoverflow.com/a/27497899/90297) for providing you with
