...
For a one-off job, spending several minutes is more than OK. But for a CI job that runs on every single commit, it is completely unacceptable. So I reached for the 2 very old tricks used in every
...
used [Vercel Functions][18] that caused crashes. These were crashing with an `EMFILE` file error, indicating that the file descriptor limit was reached. On hindsight, this is very obvious but
...
so another old friend, [`p-limit`][19][^3], came to the rescue. With a limit of 200 concurrent promises, we sailed on smoothly.
...
where we copy everything in place when we find them. This chopped another 3-4 minutes off the build time when only few files changed, which is the common case.
